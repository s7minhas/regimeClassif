...
tmp=ggplot()+geom_point(data=ggData, aes(x=xA, y=yA, pch=class, color=class), size=2, alpha=.5)
tmp=tmp+geom_abline(data=lineData, aes(intercept=ints, slope=slope), linetype=c(1,2,2))
tmp=tmp+geom_point(data=suppData, aes(x=xS, y=yS, pch=cS, color=cS), size=2)

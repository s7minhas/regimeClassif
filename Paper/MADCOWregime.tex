...
With the textual data collected, the next step is to extract the relevant features from these documents, so that we can construct a structured dataset to use in classification. The rows of this dataset consist of all documents associated with a particular country-year (i.e., in this case a collated version of the relevant State Department and Freedom House report). 

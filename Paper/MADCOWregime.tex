...
Given the high performance of the classifiers, the key contribution this project makes is in the construction of datasets that can now be used for analysis. Further these methods have not been combined in a way to produce a pipeline through which textual data can be selected, processed, converted to a high dimensional structured data format, and then used to generate regime type scores.

% Need to add something about how this was just a test case to show that these methods are feasible tools to generate measures of regime types. The real goal has been to produce the datasets which we have also done. 
...
We use State Department annual reports on countries' human rights practices from 1999 to 2013 and Freedom House's Freedom in the World reports from 1999 to 2014, which cover the period 1998-2013. These are all of the reports in the two series that are currently archived online on the source organizations' web sites.\footnote{Freedom House recently released 2015 its 2015 Freedom in the World report, however, only a few country-level reports are currently available. Once reports for all countries has been released then these reports can be ingested into our text processing pipeline.}

\section{Preparing Texts}

With this textual data in hand, we construct a structured dataset for topic modeling by . The goal of this preprocessing step is to select meaningful textual features . 
